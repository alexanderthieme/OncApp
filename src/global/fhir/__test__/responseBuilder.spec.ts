...
let exampleOncAppQuestionnaire: FHIRQuestionnaire = {
...
let validOncAppResponse0: FHIRQuestionnaireResponse = {
  ...EXAMPLE_COVAPP_RESPONSE_0,
  status: EXAMPLE_COVAPP_RESPONSE_0.status as FHIRQuestionnaireResponseStatus,
};

let validOncAppResponse1: FHIRQuestionnaireResponse = {
...
describe('Create FHIR questionnaire response', () => {
  it('should return a valid response', async () => {
    const response = buildQuestionnaireResponse(
      { ...answers0 },
      { ...exampleOncAppQuestionnaire }
    );

    delete response['authored'];
    delete validOncAppResponse0['authored'];

    expect(response).toStrictEqual(validOncAppResponse0);
  });
  ...
  it('should return a valid response with correctly formated dates', async () => {
    ...
    delete response['authored'];
    delete validOncAppResponse1['authored'];

    expect(response).toStrictEqual(validOncAppResponse1);
  });
});
